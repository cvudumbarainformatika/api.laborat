...
class DiagnosaHcController extends Controller
{
    ...
    public function simpandiagnosa(Request $request)
    {
        ...
        if ($request->has('id')) {
            $simpandiagnosa = Diagnosa::where(['id' => $request->id])->first();
            $simpandiagnosa->update(
                [
                    'rs1' => $request->noreg,
                    'rs2' => $request->norm,
        ...
        $simpandiagnosa->load('masterdiagnosa');
    ...
    public function hapusdiagnosa(Request $req)
    {
        // return new JsonResponse([
        //     $req->id
        // ], 410);
        $cari = Diagnosa::find($req->id);
        if (!$cari) {
        ...
        // $data = Diagnosa::where('rs1', $req->noreg)->with('masterdiagnosa')->get();
        // $inacbg = EwseklaimController::ewseklaimrajal_newclaim($request->noreg);
        return new JsonResponse(
            [
