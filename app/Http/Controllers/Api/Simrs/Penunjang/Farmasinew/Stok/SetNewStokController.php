...
class SetNewStokController extends Controller
{
    ...
    public function cekHargaGud()
    {
        ...
        $obDFo = FarmasinewStokreal::where('kdruang', $dFo)->whereNotIn('kdobat', $obFo)->distinct('kdobat')->get();
        $obDep = FarmasinewStokreal::whereIn('kdruang', $dep)->whereNotIn('kdobat', $obKo)->distinct('kdobat')->get();
        $stok = [];
        if (count($obDFo) > 0) {
