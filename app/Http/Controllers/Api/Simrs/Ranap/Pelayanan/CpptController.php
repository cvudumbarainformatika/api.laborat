...
use App\Models\Simrs\Ranap\Pelayanan\NotasiDpjp;
...
class CpptController extends Controller
{
  ...
  public static function cekTarip($spesialis, $request, $user)
  {
    ...
    //     $rsx = Rstigapuluhtarif::whereIn('rs3', $kode)
    //         ->where('rs4', 'like', "%|{$request->kdgroup_ruangan}|%")
    //         ->where('rs5', 'like', "%|{$request->kelas_ruangan}|%")
    //         ->get();
    // }
  ...
  public function simpanNotasiDpjp(Request $request)
  {

    // return new JsonResponse(['message' => 'Data Berhasil Disimpan', 'data' => $request->all()], 200);
    $user = Pegawai::find(auth()->user()->pegawai_id);
    $kdpegsimrs = $user->kdpegsimrs;
