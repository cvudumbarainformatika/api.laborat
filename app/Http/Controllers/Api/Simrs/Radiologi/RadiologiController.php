...
use App\Helpers\RadiologyHelper;
use App\Http\Controllers\Controller;
use App\Models\Simrs\Penunjang\Radiologi\HasilRadiologi;
use App\Models\Simrs\Penunjang\Radiologi\Pacs;
...
class RadiologiController extends Controller
{
    ...
    public function terimapasienradiologi(Request $request)
    {
        ...
        $kelamin = trim($request->kelamin) === 'Laki-laki' ? 'L' : 'P';
        ...
        foreach ($items as $item) {
            ...
            RadiologyHelper::sendPatientToPacs([
                "nota"     => $notrans,
                "noreg"    => $noreg,
                "norm"     => $norm,
                "nama"     => $nama,
                "tgllahir" => $tgllahir,
                "kelamin"  => $kelamin,
                "kdpemeriksaan" => $data['rs4'] // cukup ini
            ]);
        }
    ...
    {
        try {
            // 1. Validasi Data yang masuk dari Middleware
            $data = $request->validate([
                'nota'      => 'required', // Nomor Nota/Order
                'noreg'     => 'required', // No Reg Pasien
                'norm'      => 'required', // No RM Pasien
                'status'    => 'required', // Akan berisi 'COMPLETED'
            ...
            Pacs::updateOrCreate(
                ['nota' => $data['nota']], // Kunci pencarian (unik)
                [
                    'noreg'      => $data['noreg'],
                    'norm'      => $data['norm'],
                    'status'    => $data['status'],
            ...
            DB::commit();
            Log::info("Radiologi Berhasil Update: Nota {$data['nota']} untuk Pasien {$data['norm']}");
            ...
            Log::error("GAGAL Update Webhook Radiologi: " . $e->getMessage(), [
                'payload' => $request->all()
            ]);
            return response()->json([
                'status'  => 'error',
