...
class HemodialisaController extends Controller
{
    ...
    public function query_table_new()
    {
        ...
        $data = Lain::query();
        $select = $data->select(
            DB::raw('coalesce(rs107.rs1, rs17.rs1) as rs1'),
            DB::raw('coalesce(rs107.rs1, rs17.rs1) as noreg'),
            // 'rs107.rs1',
            'rs107.rs2',
            // 'rs107.rs1 as noreg',
        ...
        AS usia'),
            ...
            ->leftjoin('rs17', 'rs17.rs1', '=', 'rs107.rs1') //rajal
            ->leftjoin('rs23', 'rs23.rs1', '=', 'rs107.rs1') //ranap
            ...
            ->leftjoin('rs9', 'rs9.rs1', '=', 'rs107.rs14') //sistembayar
        ;


        $q = $select
            ->where(function ($w) use ($tgl, $tglx) {
                $w->whereBetween('rs107.rs3', [$tgl, $tglx])
                    ->orWhereBetween('rs17.rs3', [$tgl, $tglx]);
            })
            // ->where('rs107.rs2', '!=', '')
            ...
            ->where(function ($sts) use ($status) {
                if ($status !== 'Semua') {
                    if ($status === 'Terlayani') {
                        $sts->where(function ($r) {
                            $r->where('rs107.rs9', '=', '2')->orWhere('rs17.rs19', '=', '1');
                        });
                    } else {
                        $sts->where(function ($x) {
                            $x->whereIn('rs107.rs9', [null, '', '1'])
                                ->orWhereIn('rs17.rs9', [null, '', '2']);
                        });
                    }
                } else {
                    $sts->whereIn('rs107.rs9', [null, '', '1', '2', '3'])->orWhereIn('rs17.rs9', [null, '', '1', '2']);
                }
            })
    ...
    public function query_table()
    {
        ...
        $result = DB::table(DB::raw("({$q->toSql()} UNION ALL {$permintaan->toSql()}) as fisio"))
            ...
            ->mergeBindings($permintaan->getQuery())
    ...
    static function permintaanFisio($tgl, $tglx, $sort, $status)
    {
        ...
        AS usia'),
            ...
            ->leftjoin('listkirimcasmixRajal', 'listkirimcasmixRajal.noreg', 'rs107.rs1');
    ...
    public function newTerima(Request $request)
    {
        /**
         * data yang berbeda antara ranap dan rajal
         * 1. rajal berdasarkan noreg, ranap berdasarkan no-permintaan
