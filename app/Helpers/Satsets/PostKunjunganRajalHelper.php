...
class PostKunjunganRajalHelper
{
    ...
    public static function form($request, $pasien_uuid, $practitioner_uuid)
    {
        ...
        $radiologis = self::radiologi($request, $encounter, $tgl_kunjungan, $practitioner, $pasien_uuid, $organization_id);
        $anamnesis = self::anamnesis($request, $encounter, $tgl_kunjungan, $practitioner, $pasien_uuid);
        ...
        // PUSH RADIOLOGI (VERSI FIX 100%)
        if (count($radiologis) > 0) {
            foreach ($radiologis as $orderRadiologi) {
                // Masukkan utuh sebagai satu paket entry
                array_push($body['entry'], $orderRadiologi);
            }
        }
    ...
    static function radiologi($request, $encounter, $tgl_kunjungan, $practitioner_uuid, $pasien_uuid, $organization_id)
    {
        $entries = [];
        $radiologis = $request->radiologi;

        foreach ($radiologis as $radiologi) {
            $nota_simrs = $radiologi['rs2'];
    ...
    static function riwayatPengobatan($request, $encounter, $tgl_kunjungan, $practitioner_uuid, $pasien_uuid)
    {
        ...
        $data = null;
        // Ambil isi datanya dulu

        if (isset($send['total']) && $send['total'] > 0) {
            // if (!$telaah['petugas']) {
            //     $data=null;
            // }
